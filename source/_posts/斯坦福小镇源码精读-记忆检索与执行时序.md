...
有的解读说一条记忆就四个字段：`content / timestamp / importance / embedding`。这其实过度简化了。看官方 `ConceptNode` 的真实定义：
...
注意新鲜度的真实实现：它不是某些二手资料说的 `1 / (0.01·Δt + 1)`，而是**对按 `last_accessed` 排序后的节点，按排名 `i` 做 `0.99^i` 的指数衰减**（论文里写的是按「沙盒游戏内小时数」衰减、衰减因子 0.995；开源代码改成了按排名、0.99，二者实现并不完全一致）。而且这里的**排序方向**藏着一个反直觉的坑——本节末尾专门拆（见「深挖：新鲜度排序方向其实是反的」）。
...
所以那套「**累乘塌陷**」的论证——「3 个月前一条 importance=10 的核心记忆，因为新鲜度趋近 0，三项一乘总分归零，于是被永久遗忘」——是建立在一个**根本不存在的乘法**上的。加权和里，某一项为 0 只是少加一块，另外两项照样把分数顶上去，不会「归零」。

更有意思的是，常见的「优化建议」是「把累乘换成加权求和、弱化时间影响」。可官方**本来就是加权和**，而且：

- `scratch.py` 默认 `recency_w = relevance_w = importance_w = 1`；
- `retrieve.py` 里再乘一层 `gw = [0.5, 3, 2]`。

合起来，三项的有效权重是 **新鲜度 0.5、相关性 3、重要度 2**。也就是说，官方**早就把新鲜度压到了最低权重**，相关性和重要度才是主导。这个建议兜了一圈，得到的正是官方已有的设计。
...
可要是按那套**累乘** `R×Rel×Imp` 呢？M2 = `0.00 × 0.56 × 0.89 = 0` 💥——只因新鲜度=0，它 0.89 的高重要度、0.56 的相关性全被"一票否决"，彻底丢失。这就是"累乘塌陷"的来历。**官方是加法，M2 活得好好的；那个会塌陷的乘法根本不存在。**
...
- 官方仓库：[`joonspk-research/generative_agents`](https://github.com/joonspk-research/generative_agents)（以 `main` 分支为准）
- 认知模块：`reverie/backend_server/persona/cognitive_modules/` 下的 `retrieve.py`、`perceive.py`、`plan.py`、`reflect.py`、`converse.py`
- Persona 主类：`reverie/backend_server/persona/persona.py`
- 记忆结构：`reverie/backend_server/persona/memory_structures/` 下的 `associative_memory.py`、`scratch.py`
- Prompt 相关：`reverie/backend_server/persona/prompt_template/` 下的 `run_gpt_prompt.py`、`gpt_structure.py`，以及 `v1` / `v2` / `v3_ChatGPT` 目录下的 `.txt` 模板
- 主循环：`reverie/backend_server/reverie.py`
- 作者 2024 年重写版：[`StanfordHCI/genagents`](https://github.com/joonspk-research/genagents)
- 论文：*Generative Agents: Interactive Simulacra of Human Behavior*（Park et al., UIST 2023, arXiv:2304.03442）
